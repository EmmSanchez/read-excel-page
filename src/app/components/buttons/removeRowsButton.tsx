...
export function RemoveRowsButton({ selectedRows, deleteSelectedRows, isPopoverVisible, setIsPopoverVisible, setRowToDelete }: RemoveRowsButtonProps) {
  ...
  return (
    <>
      <button onClick={handleDeleteClick} disabled={selectedRows.length === 0} className={`w-[140px] px-4 py-[6px] rounded-md text-xl text-[#2563EB] font-semibold border-[1.4px] border-solid border-gray-300 transition-all ${selectedRows.length === 0 ? 'text-[#E2E8F0] cursor-not-allowed' : 'hover:bg-gray-100'}`}>
