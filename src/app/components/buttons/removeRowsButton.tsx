...
export function RemoveRowsButton({ selectedRows, deleteSelectedRows, isPopoverVisible, setIsPopoverVisible, setRowToDelete }: RemoveRowsButtonProps) {
  ...
  return (
    <>
      <button onClick={handleDeleteClick} disabled={selectedRows.length === 0} className={`w-[140px] px-4 py-[6px] rounded-md text-xl text-[#2563EB] dark:text-slate-100 font-semibold border-[1.4px] border-solid border-gray-700 dark:border-slate-700 transition-all ${selectedRows.length === 0 ? 'opacity-15 cursor-not-allowed' : 'dark:border-gray-100 hover:bg-gray-100 dark:hover:bg-slate-800'}`}>
      ...
      <div className={`absolute w-[500px] -left-[500px] top-16 bg-white dark:bg-[#1D1E2B] rounded-md outline outline-gray-300 dark:outline-gray-600 outline-1 drop-shadow-md p-4 text-base transition-all ease-in-out duration-100 transform ${isPopoverVisible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 pointer-events-none scale-[.98] -translate-y-1'}`}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-2">
            <button onClick={handleCancelClick} className='px-4 py-2 rounded-md text-lg border-[1px] border-solid border-[#cecbcb] hover:bg-gray-200 dark:hover:bg-[#14151e]'>Cancelar</button>
